...
def generate_answer(user_query, context_documents, chat_history):
    ...
    conversation_history = "\n".join([f"{m['role']}: {m['content']}" for m in chat_history[-3:]])  # Keep recent history
...
async def async_generate_answer(user_query):
    ...
    related_docs = await loop.run_in_executor(None, find_related_documents, user_query, st.session_state.chat_history)
    response = await loop.run_in_executor(None, generate_answer, user_query, related_docs, st.session_state.chat_history)
    return response
